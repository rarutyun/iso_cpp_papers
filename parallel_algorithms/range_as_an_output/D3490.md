...
Intel(R) oneAPI DPC++ Library (oneDPL) [developer
guide](https://oneapi-src.github.io/oneDPL/parallel_api/parallel_range_algorithms.html) covers parallel range algorithms
we've implemented so far. We chose range-as-an-output approach where applicable: `transform`, `copy`, `copy_if`,
and `merge`.

We don't have overloads with iterator-as-an-output for the mentioned algorithms since ranges make more sense for parallelism
from our standpoint. However, we don't forsee any issues to add extra overloads with iterators later, if necessary.
