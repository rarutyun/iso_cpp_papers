...
is a one caveat, though.

Not adding the range-as-an-output overloads has a direct impact `Iterator` + `Sentinel` overloads because we cannot define
a bounded version of this overload that range overload would map to.

So, the signature would look like the serial range algorithm counterpart. Taking `transform` algorithm as the example the
difference to what is proposed in the paper would look like the following:

```cpp
template<typename ExecutionPolicy,
         random_access_iterator I1, sentinel_for<I1> S1,
         random_access_iterator I2, sentinel_for<I2> S2,
         random_access_iterator O, @[`sized_sentinel_for<O> SO,`]{.rm}@
...
constexpr binary_transform_result<I1, I2, O>
    transform(ExecutionPolicy&& policy, I1 first1, S1 last1, I2 first2, S2 last2, O result,
              @[`SO result_last,`]{.rm}@ F binary_op, Proj1 proj1 = {}, Proj2 proj2 = {});
```

Given that the committee already has concerns about the amount of algorithm overloads we will likely end up with having
just unbound version for parallel range overloads for `Iterator` + `Sentinel`, the one that is shown above.
