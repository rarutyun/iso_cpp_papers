...
This paper proposes reconsidering the design for parallel `ranges::rotate_copy` and `ranges::reverse_copy` in [@P3179R9].
...
As a reminder, one of the design goals for [@P3179R9] was to keep the same return type for the proposed algorithms
...
In [@P3179R9] `rotate_copy` returns the iterator past the last copied element for input. By design, it never returns `last`,
