...
The raised concern is that users might be surprised at runtime by a completely different return value after just adding an
...
// res.in is "last" (the iterator past the end of data)
...
For `reverse_copy` the proposed algorithm in [@P3179R7] always returns the iterator past the last copied element (in the reversed order) for the input.
