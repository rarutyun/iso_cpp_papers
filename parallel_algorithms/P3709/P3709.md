...
LEWG approved the "range-as-the-output" design aspect for our parallel range algorithms proposal [@P3179R7].
It introduces "range-as-the-output" with its own bound, taking into account that the output size may be less than the input size.
A common questions for such algorithms is what to return for the input when the output size is not sufficient to hold the processed data?
...
// res2.in point to the element equal to 4
```
...
The proposed design in [@P3179R7] follows the same logic (returning the stop point for the input) for parallel
...
- `rotate_copy` uses two subranges within the input range, split by `middle`. The one past the last iterator for the input
  also never equals to `last` : it's one of the iterators in either `[middle, last - 1]` or `[first, middle]`.

The proposed design in [@P3179R7] allows users to know where the algorithm stops if the
...
much information as possible. For example, `ranges::for_each` returns the iterator equal to `last` because that is what the
...
As a reminder, one of the design goals for [@P3179R7] was to keep the same return type for the proposed algorithms
...
In [@P3179R7] `rotate_copy` returns the iterator past the last copied element for input. By design, it never returns `last`,
...
The raised concern is that users might be surprised at runtime by a completely different return value while just changing
how the output range is passed in. Consider the following example:
...
```cpp
std::list in_v{1,2,3,4,5,6};
std::vector<int> out_v(6);

// in_view is not common_range
auto in_view = std::views::counted(in_v.begin(), 6);
...
// iterator as the output
auto res = std::ranges::rotate_copy(in_view, middle, out_v.begin());
...
// range as the output
auto res = std::ranges::rotate_copy(in_view, middle, out_v);
...
Since `rotate_copy` has two subranges - from `middle` to `last` and from `first` to `middle` - we want to tell how far
...
The updated signatures are below:
...
For `reverse_copy` the proposed algorithm in [@P3179R7] always returns the iterator to the last copied element (in the reversed order)
...
// iterator as the output
auto res = std::ranges::reverse_copy(in_view, out_v.begin());
...
// range as the output
auto res = std::ranges::reverse_copy(in_view, out_v);
...
- `in2` represents the stop point, as proposed in [@P3179R7].
...
- Keep the [@P3179R7] status quo. The concerns are:
...
parallel `reverse_copy` and `rotate_copy` from [@P3179R7] and add them later, possibly together with the serial
...
::: draftnote
TODO: The return value is defined incorrectly, should be fixed before submission!
:::
