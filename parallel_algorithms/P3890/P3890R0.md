...
## Modify [specialized.algorithms.general]{- .sref}

[3]{.pnum}[When new objects are created by the algorithms specified in [specialized.algorithms]{- .sref}, the lifetime ends
for any existing objects (including potentially-overlapping subobjects [intro.object]{- .sref}) in storage that is reused
[basic.life]{- .sref}.]{.note}

[4]{.pnum} Some algorithms specified in [specialized.algorithms]{- .sref} make use of the following exposition-only function
templates:
...
template<class I>
  decltype(auto) @_deref-move_@(I& it) {
...
[5]{.pnum} [For the parallel algorithm overloads in subclause [specialized.algorithms]{- .sref}, each iteration of the loop
with a `new` expression or invocation of `destroy`" in the specification of the function template (if any) is
considered to be an invocation of an element access function ([algorithms.parallel.exec]{- .sref}).]{.add}
...
```cpp
  for (; first != last; (void)++result, ++first)
    ::new (voidify(*result))
      typename iterator_traits<NoThrowForwardIterator>::value_type(@_deref-move_@(first));
  return result;
```
...
[3]{.pnum} *Preconditions*: [`ofirst`, `olast`) does not overlap with [`ifirst`, `ilast`).
...
[5]{.pnum} [If an exception is thrown, some objects in the range \[`ifirst`, `ilast`) are left in a valid, but unspecified state.]{.note}
...
[8]{.pnum} *Preconditions*: [`ofirst`, `olast`) does not overlap with `ifirst` + [0, `n`).
...
[10]{.pnum} [If an exception is thrown, some objects in the range `ifirst` + \[0, `n`) are left in a valid but unspecified state.]{.note}
...
- [1.1]{.pnum} If `T` is an array type, equivalent to `destroy(begin(*location), end(*location))`.
- [1.2]{.pnum} Otherwise, equivalent to `location->~T()`.
