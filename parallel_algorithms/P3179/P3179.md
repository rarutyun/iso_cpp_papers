...
With the output sequences being bounded, there is a possibility that some elements of input sequences remain
not processed by an algorithm, and it makes sense for it to return an iterator to the position in the input
where the processing stopped. Because of that, for several algorithms described below we propose modifications
to which iterators are returned that do not match their existing serial counterparts. 
...
```cpp
std::unique_ptr<int[]> mem = std::make_unique_for_overwrite<int[]>(10);
...
If some day serial range algorithms with range-as-the-output appear, we envision the same semantics for them.
