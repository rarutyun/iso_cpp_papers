...
- If `f` returns a result, the result is ignored. Implementations do not have the freedom granted under
...
[2]{.pnum} *Preconditions*: `op` and `binary_op` [satisfy the requirements specified in
[algorithms.parallel.user]{- .sref},]{.add} do not invalidate iterators or subranges, nor modify elements in the ranges
...
- [4.2]{.pnum} `{last1, @[result_last]{.rm}[result + _N_]{.add}@}` for the overloads in namespace
  `ranges`[, if *`N`* is equal to *`M`* is `true`]{.add}.
...
- [4.2]{.pnum} ``{last1, last2, @@[result_last]{.rm}[`result + @_N_@`]{.add}@@}`` for the overloads in
  namespace `ranges`[, if *`N`* is equal `@_K_@ + @_M_@`]{.add}.
- [4.x]{.pnum} [Otherwise, `{j1, j2, result_last}` for the overloads in namespace `ranges`,
