...
when the guarantee says “at most *expr*” or “exactly *expr*” and does not specify the number of assignments or swaps, and
...
[3]{.pnum} *Complexity*: For the overloads with no [`ExecutionPolicy`]{.rm}[execution policy template parameter]{.add}, exactly
min(`(i - first) + 1`,  `(last - first) - 1`) applications of the corresponding predicate, where `i` is `adjacent_find`'s
return value. For the overloads with an [`ExecutionPolicy`]{.rm}[execution policy template parameter]{.add}, _O_(`last - first`)
