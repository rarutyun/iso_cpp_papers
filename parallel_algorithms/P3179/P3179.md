...
## Corner cases: `rotate_copy` and `reverse_copy` {#corner_cases}

// TODO: add design rational.
...
- [x.6]{.pnum} *`K`* be the rightmost position of an element `e1` in `[first1, last1)` such that
  the number of elements `e2` in `[first2, last2)` for which *`E`* holds is less than or equal to *`N - K`*.
:::
...
[3]{.pnum} *Effects*: Constructs a sorted union of the elements from [the]{.rm}[*`N`*]{.add} two ranges; that is, the set of
...
- [4.2]{.pnum} [`{last1, last2, result_last}`]{.rm}[`{j1, j2, result + @_N_@}`]{.add} for the overloads in namespace `ranges`[,
  where the iterators `j1` and `j2` point to positions past the last copied elements in `[first1, last1)` and `[first2, last2)`
...
- [4.1]{.pnum} `result_last` for the overloads in namespace `std`.
- [4.2]{.pnum} [`{last1, last2, result_last}`]{.rm}[`{j1, j2, result + @_N_@}`]{.add} for the overloads in namespace `ranges`[,
  where the iterators `j1` and `j2` point to positions past the last copied element in `[first1, last1)` and `[first2, last2)`
  respectively if `result + @_N_@ == result_last`, and `j1 == last1`, `j2 == last2` otherwise]{.add}.
...
range `[first2, last2)` to the range beginning at `result`. The elements in the constructed range are sorted.

[4]{.pnum} *Returns*: [Let `result_last` be the end of the constructed range. Returns]{.rm}

- [4.1]{.pnum} `result_last` for the overloads in namespace `std`.
- [4.2]{.pnum} [`{last1, result_last}`]{.rm}[`{j1, result + @_N_@}`]{.add} for the overloads in namespace `ranges`[,
  where the iterator `j1` points to position past the last copied element in `[first1, last1)` if
