...
- [1.x]{.pnum} *`N`* be `min(result_last - result,  @_M_@)`;
:::
- [1.3]{.pnum} *`E`* be
...
to the overload with an [`ExecutionPolicy`]{.rm}[execution policy]{.add}.
...
[11]{.pnum} *Complexity*: Exactly [`last - first`]{.rm}[`first + @_N_@`]{.add} applications of the corresponding predicate
and any projection.
...
- [13.2]{.pnum} `{@[last]{.rm}[i]{.add}@, result + @_N_@}`, for the algorithms in namespace `ranges`[, where `i` points to a
  position past the last copied element if `result + @_N_@ == result_last`, and `last` otherwise]{.add}.
