...
- [x.1]{.pnum} *`M`* be `last - first`;
- [x.2]{.pnum} *`N`* be `min(@_M_@, result_last - result)`;
- [x.2]{.pnum}*`NEW_MIDDLE`* be `first + ((@_N_@ + (middle - first)) % @_M_@)`.
...
- [x.1]{.pnum} `result + @_N_@` for the overloads in namespace `std`.
- [x.2]{.pnum} `{@_NEW_MIDDLE_@, result + @_N_@}` for the overload in namespace `ranges`.
...
respectively if `result + N == result_last`, and `j1 == last1`, `j2 == last2` otherwise]{.add}.
...
then all [*`m`*]{.rm}[such]{.add} elements from the first range are copied to the output range, in order, and then
