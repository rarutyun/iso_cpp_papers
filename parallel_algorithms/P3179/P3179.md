...
- Decide on `rotate_copy`
- ~~Decide if `fill` should use `output_iterator` or not~~
...
+      rotate_copy(Ep&& exec, R&& r, iterator_t<R> middle, OutR&& result_r);
}
...
- [x.1]{.pnum} `result_last` be `first + (last - first)` for the overloads with no parameter `result_last` or `result_r`;
- [x.2]{.pnum} *`M`* be the number the first element from every consecutive group of equal elements in the range
...
[9]{.pnum} *Effects*: Copies [only the]{.rm}[*`N`*]{.add} first element[s]{.add} from every consecutive group of equal
...
no more than twice asmany applications of any projection.
...
[x]{.pnum} `result_last` be `first + (last - first)` for the overloads with no parameter `result_last` or `result_r`;
:::

[5]{.pnum} Let *`N`* be [`last - first`]{.rm}[`min(last - first, result_last - result)`]{.add}.

[6]{.pnum} *Preconditions*: The ranges ``[first, @@[last]{.rm}[`first + @_N_@`]{.add}@@)`` and `[result, result + @_N_@)`
do not overlap.

[7]{.pnum} *Effects*: Copies the range ``[first, @@[last]{.rm}[`first + @_N_@]{.add}@@)`` to the range
`[result, result + @_N_@)` such that for every non-negative integer `i < @_N_@` the following assignment takes place:
`*(result + @_N_@ - 1 - i) = *(first + i)`.

[8]{.pnum} *Returns*:

- [8.1]{.pnum} `result + @_N_@` for the overloads in namespace `std`.
- [8.2]{.pnum} ``{@@[last]{.rm}[`first + @_N_@`]{.add}@@, result + @_N_@}`` for the overloads in namespace `ranges`.
...
- [x.3]{.pnum} *`N`* be `min(@_M_@, result_last - result)`;
- [x.4]{.pnum} `result_last` be `result + @_N_@` for the overloads with no parameter `result_last` or `result_r`;
- [x.5]{.pnum} *`E`* be `bool(invoke(comp, invoke(proj2, e2), invoke(proj1, e1)))`;
- [x.6]{.pnum} *`K`* be the number of elements for which *`E`* holds.
:::
...
`[result, result_last)`, where `result_last` is `result + @_N_@`]{.rm}[Copies `[first1 + @_N_ - _K_@)` elements of the
range `[first1, last1)` and `[first2 + @_K_@)` elements of the range `[first2, last2)`]{.add} into the range
...
- [4.2]{.pnum} [`{last1, last2, result_last}`]{.rm}[`{first1 + @_N_ - _K_@, first2 + @_K_@, result + @_N_@}`]{.add}
