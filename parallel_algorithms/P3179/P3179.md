...
- The parallel range algorithms support all standard and implementation-defined execution policies. ([[#supported_policies]])
...
template<@_execution-policy_@ Ep, ranges::random_access_range R, ranges::random_access_range OutR,
         copy_constructible F, class Proj = identity>
  requires indirectly_writable<ranges::iterator_t<OutR>, indirect_result_t<F&, projected<ranges::iterator_t<R>, Proj>>>
           && sized_range<R> && sized_range<OutR>
...
- `generate_random` (RNG specific, `ExecutionPolicy` parameter was already discussed during [@P1068R11] review)
...
- Update <a href="https://eel.is/c++draft/algorithm.syn">[algorithm.syn]</a>.
- Consider adding the algorithm descriptions to the wording, even if no changes are made.

<!-- Add "The intention is that algorithms call begin/end only once, in serial code" to the design notes. -->
...
## Add the feature test macro to <a href="http://eel.is/c++draft/version.syn">[**version.syn**]</a> ## {#modify_version_syn}

```cpp
#define __cpp_lib_parallel_algorithm                201603L // also in <algorithm>, <numeric>
@[`#define __cpp_lib_parallel_range_algorithms         20����L // also in <algorithm>, <memory>`]{.add}@
...
Unless otherwise specified, [function]{.rm}[invocable]{.add} objects passed into parallel algorithms as objects of type
...
## Modify <a href="https://eel.is/c++draft/algorithms.parallel.overloads">[**algorithms.parallel.overloads**]</a> ## {#modify_alg_parallel_overloads}
...
::: add
```cpp
template<@_execution-policy_@ ExecutionPolicy, random_access_iterator I, sized_sentinel_for<I> S, class Proj = identity,
         indirect_unary_predicate<projected<I, Proj>> Pred>
  bool ranges::all_of(ExecutionPolicy&& exec, I first, S last, Pred pred, Proj proj = {});
template<@_execution-policy_@ ExecutionPolicy, random_access_range R, class Proj = identity,
         indirect_unary_predicate<projected<iterator_t<R>, Proj>> Pred>
  requires sized_range<R>
  bool ranges::all_of(ExecutionPolicy&& exec, R&& r, Pred pred, Proj proj = {});
```
:::
...
template<@_execution-policy_@ ExecutionPolicy, random_access_range R, class Proj = identity, class T = projected_value_t<iterator_t<R>, Proj>>
  requires indirect_binary_predicate<ranges::equal_to, projected<iterator_t<R>, Proj>, const T*> &&
    sized_range<R>
  bool ranges::contains(ExecutionPolicy&& exec, R&& r, const T& value, Proj proj = {});
```

_Returns_: `ranges​::​find(std::forward<ExecutionPolicy>(exec), std​::​move(first), last, value, proj) != last.`
:::
...
::: add
```cpp
template<@_execution-policy_@ ExecutionPolicy, random_access_iterator I1, sized_sentinel_for<I1> S1,
         random_access_iterator I2, sized_sentinel_for<I2> S2,
         class Pred = ranges::equal_to, class Proj1 = identity, class Proj2 = identity>
  requires indirectly_comparable<I1, I2, Pred, Proj1, Proj2>
  bool ranges::contains_subrange(ExecutionPolicy&& exec, I1 first1, S1 last1, I2 first2, S2 last2,
                                 Pred pred = {}, Proj1 proj1 = {}, Proj2 proj2 = {});
template<@_execution-policy_@ ExecutionPolicy, random_access_range R1, random_access_range R2,
         class Pred = ranges::equal_to, class Proj1 = identity, class Proj2 = identity>
  requires indirectly_comparable<iterator_t<R1>, iterator_t<R2>, Pred, Proj1, Proj2> &&
...
::: add
```cpp
template<@_execution-policy_@ ExecutionPolicy, random_access_iterator I, class Proj = identity,
         indirectly_unary_invocable<projected<I, Proj>> Fun>
...
::: add
```cpp
template<@_execution-policy_@ ExecutionPolicy, random_access_iterator I1, sized_sentinel_for<I1> S1, random_access_iterator I2,
...
template<@_execution-policy_@ ExecutionPolicy, random_access_iterator I1, sized_sentinel_for<I1> S1,
  ...
  bool ranges::equal(ExecutionPolicy&& exec, I1 first1, S1 last1, I2 first2, S2 last2,
                     Pred pred = {}, Proj1 proj1 = {}, Proj2 proj2 = {});
template<@_execution-policy_@ ExecutionPolicy, random_access_range R1, random_access_range R2, class Pred = ranges::equal_to,
         class Proj1 = identity, class Proj2 = identity>
  requires indirectly_comparable<iterator_t<R1>, iterator_t<R2>, Pred, Proj1, Proj2> &&
    sized_range<R1> && sized_range<R2>
...
::: add
```cpp
template<@_execution-policy_@ ExecutionPolicy, random_access_iterator I, sized_sentinel_for<I> S,
         class Pred = ranges::equal_to, class Proj = identity,
         class T = projected_value_t<I, Proj>>
  requires indirectly_comparable<I, const T*, Pred, Proj>
    subrange<I>
...
template<@_execution-policy_@ ExecutionPolicy, random_access_range R, class Pred = ranges::equal_to,
         class Proj = identity, class T = projected_value_t<iterator_t<R>, Proj>>
  requires indirectly_comparable<iterator_t<R>, const T*, Pred, Proj>
           && sized_range<R>
...
_Returns_: `ranges::mismatch(std::move(first1), last1, std::move(first2), last2, pred, proj1, proj2).in2 == last2`
...
::: add
```cpp
template<@_execution-policy_@ ExecutionPolicy, random_access_iterator I1, sized_sentinel_for<I1> S1, random_access_iterator I2, sized_sentinel_for<I2> S2,
         class Pred = ranges::equal_to, class Proj1 = identity, class Proj2 = identity>
  requires indirectly_comparable<I1, I2, Pred, Proj1, Proj2>
...
_Returns_: `false` if `N1 < N2`, otherwise `ranges::equal(views::drop(ranges::ref_view(r1), N1 - static_cast<decltype(N1)>(N2)), r2, pred, proj1, proj2)`

::: add
```cpp
template<@_execution-policy_@ Ep, random_access_range R1, random_access_range R2,
         class Pred = ranges::equal_to, class Proj1 = identity, class Proj2 = identity>
...
<i>Returns</i>: `false` if `N1 < N2`, otherwise
`ranges::equal(std::forward<Ep>, views::drop(ranges::ref_view(r1), N1 - static_cast<decltype(N1)>(N2)),
               r2, pred, proj1, proj2)`
:::
...
```cpp
template<@_execution-policy_@ Ep, class ForwardIterator1, class ForwardIterator2>
  ForwardIterator2 copy(Ep&& policy,
...
template<@_execution-policy_@ Ep, random_access_range R, random_access_range OutR>
  requires indirectly_copyable<iterator_t<R>, iterator_t<OutR>> && sized_range<R> && sized_range<OutR>
...
template<@_execution-policy_@ Ep, random_access_iterator I, sized_sentinel_for<I> S,
  ...
  ranges::copy_if_result<I, O>
    ranges::copy_if(Ep&& exec, I first, S last, O result, OutS result_last,
                    Pred pred, Proj proj = {});
template<@_execution-policy_@ Ep, random_access_range R, random_access_iterator OutR,
...
```cpp
template<@_execution-policy_@ Ep, class ForwardIterator1, class ForwardIterator2>
  ForwardIterator2 move(Ep&& policy,
                        ForwardIterator1 first, ForwardIterator1 last,
                        ForwardIterator2 result);
```
...
template<@_execution-policy_@ Ep, random_access_range R, random_access_range OutR>
  requires indirectly_movable<iterator_t<R>, iterator_t<OutR>> && sized_range<R> && sized_range<OutR>
...
template<@_execution-policy_@ Ep, random_access_range R1, random_access_range R2, random_access_range OutR,
         copy_constructible F, class Proj1 = identity, class Proj2 = identity>
...
template<@_execution-policy_@ Ep, random_access_range R, class Proj = identity,
         class T1 = projected_value_t<iterator_t<R>, Proj>, class T2 = T1>
  requires indirectly_writable<iterator_t<R>, const T2&> &&
           indirect_binary_predicate<ranges::equal_to, projected<iterator_t<R>, Proj>, const T1*> &&
           sized_range<R>
...
template<@_execution-policy_@ Ep, random_access_range R, random_access_range OutR,
  ...
  requires indirectly_copyable<iterator_t<R>, iterator_t<OutR>> &&
           indirect_binary_predicate<ranges::equal_to, projected<iterator_t<R>, Proj>, const T1*> &&
           indirectly_writable<iterator_t<OutR>, const T2&> && sized_range<R> && sized_range<OutR>
...
template<@_execution-policy_@ Ep, random_access_range R, random_access_range OutR,
         ...
         indirect_unary_predicate<projected<iterator_t<R>, Proj>> Pred>
  requires indirectly_copyable<iterator_t<R>, iterator_t<OutR>> &&
           indirectly_writable<iterator_t<OutR>, const T&> && sized_range<R> && sized_range<OutR>
...
template<@_execution-policy_@ Ep, random_access_range R, class T = range_value_t<R>>
...
template<@_execution-policy_@ Ep, random_access_range R, copy_constructible F>
...
template<@_execution-policy_@ Ep, random_access_range R, random_access_range OutR, class Proj = identity,
         class T = projected_value_t<iterator_t<R>, Proj>>
  requires indirectly_copyable<iterator_t<R>, iterator_t<OutR>> &&
           indirect_binary_predicate<ranges::equal_to, projected<iterator_t<R>, Proj>, const T*> &&
           sized_range<R> && sized_range<OutR>
...
template<@_execution-policy_@ Ep, random_access_range R>
  requires permutable<iterator_t<R>> && sized_range<R>
...
```
*Effects*: Equivalent to: `return ranges​::​rotate(std::forward<Ep>(exec), ranges​::​begin(r), middle,
ranges​::​end(r));`
:::
...
template<@_execution-policy_@ Ep, random_access_range R, class Comp = ranges::less, class Proj = identity>
...
`return ranges::partial_sort(std::forward<Ep>(exec), ranges::begin(r), middle, ranges::end(r), comp, proj);`
:::
...
::: add
```cpp
template<@_execution-policy_@ Ep, random_access_range I1, sized_sentinel_for<I1> S1,
         random_access_iterator I2, sized_sentinel_for<I2> S2,
         class Comp = ranges::less, class Proj1 = identity, class Proj2 = identity>
  requires indirectly_copyable<I1, I2> && sortable<I2, Comp, Proj2> &&
           indirect_strict_weak_order<Comp, projected<I1, Proj1>, projected<I2, Proj2>>
  ranges::partial_sort_copy_result<I1, I2>
    ranges::partial_sort_copy(Ep&& exec, I1 first, S1 last, I2 result_first, S2 result_last,
                              Comp comp = {}, Proj1 proj1 = {}, Proj2 proj2 = {});
template<@_execution-policy_@ Ep, random_access_range R1, random_access_range R2, class Comp = ranges::less,
         class Proj1 = identity, class Proj2 = identity>
  requires indirectly_copyable<iterator_t<R1>, iterator_t<R2>> &&
           sortable<iterator_t<R2>, Comp, Proj2> &&
           indirect_strict_weak_order<Comp, projected<iterator_t<R1>, Proj1>,
...
```
*Effects*: Equivalent to:
`return ranges::nth_element(std::forward<Ep>(exec), ranges::begin(r), nth, ranges::end(r), comp, proj);`
:::
...
template<@_execution-policy_@ Ep, random_access_range R, random_access_range OutR1,
         random_access_range OutR2, class Proj = identity,
         indirect_unary_predicate<projected<iterator_t<R>, Proj>> Pred>
  requires indirectly_copyable<iterator_t<R>, iterator_t<OutR1>> &&
           indirectly_copyable<iterator_t<R>, iterator_t<OutR2>> &&
           sized_range<R> && sized_range<OutR1> && sized_range<OutR2>
...
template<@_execution-policy_@ Ep, random_access_range R1, random_access_range R2,
         random_access_range OutR, class Comp = ranges::less,
         class Proj1 = identity, class Proj2 = identity>
...
```
*Effects*: Equivalent to:<br>
`return ranges::inplace_merge(std::forward<Ep>(exec), ranges::begin(r), middle, ranges::end(r), comp, proj);`
:::
...
template<@_execution-policy_@ Ep, random_access_iterator I1, sized_sentinel_for<I1> S1, random_access_iterator I2, sized_sentinel_for<I2> S2,
  ...
  bool ranges::includes(Ep&& exec, I1 first1, S1 last1, I2 first2, S2 last2,
                        Comp comp = {}, Proj1 proj1 = {}, Proj2 proj2 = {});
template<@_execution-policy_@ Ep, random_access_range R1, random_access_range R2, class Proj1 = identity, class Proj2 = identity,
...
template<@_execution-policy_@ Ep, random_access_range R1, random_access_range R2, class Proj1 = identity,
         class Proj2 = identity,
         indirect_strict_weak_order<projected<iterator_t<R1>, Proj1>,
                                    projected<iterator_t<R2>, Proj2>> Comp = ranges::less>
  requires sized_range<R1> && sized_range<R2>
...
<ins>
template&lt;class I>
  concept <i>nothrow-bidirectional-iterator</i> = <i>see below</i>;  <i>// exposition only</i>
template&lt;class I>
...
<ins>
template&lt;class I>
  concept <i>nothrow-bidirectional-range</i> = <i>see below</i>;     <i>// exposition only</i>
template&lt;class I>
...
<ins>
<pre class="wordingStyle">
template&lt;execution-policy ExecutionPolicy, <i>nothrow-random-access-iterator</i> I, <i>nothrow-sized-sentinel-for</i>&lt;I> S>
  requires default_initializable&lt;iter_value_t&lt;I>>
  I uninitialized_default_construct(ExecutionPolicy&& exec, I first, S last); <i>// see [algorithms.parallel.overloads]</i>

template&lt;execution-policy ExecutionPolicy, <i>nothrow-random-access-range</i> R>
...
template&lt;execution-policy ExecutionPolicy, <i>nothrow-random-access-iterator</i> I>
  requires default_initializable&lt;iter_value_t&lt;I>>
...
<ins>
<pre class="wordingStyle">
template&lt;execution-policy ExecutionPolicy, random_access_iterator I, sized_sentinel_for&lt;I> S1,
...
template&lt;execution-policy ExecutionPolicy, random_access_range IR, <i>nothrow-random-access-range</i> OR>
...
template&lt;execution-policy ExecutionPolicy, random_access_iterator I, <i>nothrow-random-access-iterator</i> O,
...
template&lt;execution-policy ExecutionPolicy, random_access_iterator I,
...
<ins>
<pre class="wordingStyle">
template&lt;execution-policy ExecutionPolicy, <i>nothrow-random-access-iterator</i> I, <i>nothrow-sized-sentinel-for</i>&lt;I> S, class T>
  requires constructible_from&lt;iter_value_t&lt;I>, const T&>
    I uninitialized_fill(ExecutionPolicy&& exec, I first, S last, const T& x);                <i>// see [algorithms.parallel.overloads]</i>
template&lt;execution-policy ExecutionPolicy, <i>nothrow-random-access-range</i> R, class T>
...
template&lt;execution-policy ExecutionPolicy, <i>nothrow-random-access-iterator</i> I, class T>
  requires constructible_from&lt;iter_value_t&lt;I>, const T&>
...
<pre class="wordingStyle">
template&lt;class S, class I>
concept <i>nothrow-sized-sentinel-for</i> = <i>// exposition only</i>
  <i>nothrow-sentinel-for</i>&lt;S, I> &&
  sized_sentinel_for&lt;S, I>; 
</pre>

Types `S` and `I` model <code><i>nothrow-sized-sentinel-for</i></code> only if no exceptions are thrown from the <code>-</code> operator
...
<pre class="wordingStyle">
template&lt;class I>
concept <i>nothrow-bidirectional-iterator</i> = <i>// exposition only</i>
  <i>nothrow-forward-iterator</i>&lt;I> &&
  bidirectional_iterator&lt;I>;
</pre>

A type `I` models <code><i>nothrow-bidirectional-iterator</i></code> only if no exceptions are thrown from decrementing valid iterators.

[*Note X*: This concept allows some `bidirectional_iterator` ([iterator.concept.bidir]) operations to throw exceptions. — *end note*]

<pre class="wordingStyle">
template&lt;class R>
concept <i>nothrow-bidirectional-range</i> = <i>// exposition only</i>
  <i>nothrow-forward-range</i>&lt;R> &&
  <i>nothrow-bidirectional-iterator</i>&lt;iterator_t&lt;R>>;
</pre>
<pre class="wordingStyle">
template&lt;class I>
concept <i>nothrow-random-access-iterator</i> = <i>// exposition only</i>
  <i>nothrow-bidirectional-iterator</i>&lt;I> &&
  random_access_iterator&lt;I> &&
  <i>nothrow-sized-sentinel-for</i>&lt;I, I>;
</pre>

A type `I` models <code><i>nothrow-random-access-iterator</i></code> only if no exceptions are thrown from advancement
...
concept <i>nothrow-random-access-range</i> = <i>// exposition only</i>
  <i>nothrow-bidirectional-range</i>&lt;R> &&
...
- Address other feedback from SG1 and SG9 and fix known bugs (see [[#issues_to_address]]):
...
references:
  - id: P3490R0
    ...
    title: "Justification for range-as-an-output for parallel range algorithms"
    author:
      - family: Kukanov
        given: Alexey
      - family: Arutyunyan
