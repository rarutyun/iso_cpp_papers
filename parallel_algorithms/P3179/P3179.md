...
```
::: add
For each iterator `i` and sentinel `s` produced from a range `r`, the semantics of `s - i` is an unspecified choice of
either `ranges::distance(r)` or `ranges::distance(i, s)`.
...
`regular_invocable` (except `for_each`, `for_each_n`, `generate`, `generate_n`, and `transform` in namespace `ranges`),]{.add}
