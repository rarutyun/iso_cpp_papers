...
applications of the corresponding predicate, and no more than twice as many applications of any projection.
...
[8]{.pnum} *Returns*: `result + @[(last - first)]{.rm}@@[_N_]{.add}@`.
...
- [13.2]{.pnum} `{first + @_N_@, result + @_N_@}` for the overload in namespace `ranges`.
...
- [x]{.pnum} `result_last` be `first + (last - first)` for the overloads with no parameter `result_last`.
- [x]{.pnum} *`E`* be:

  - [x.x]{.pnum} `bool(pred(*i))` for the overloads in namespace `std`;
  - [x.x]{.pnum} `bool(invoke(pred, invoke(proj, *i)))` for the overloads in namespace `ranges`,

- [x]{.pnum} *`M`* be the number of iterators `i` in the range `[first, last)` for which the condition *`E`* holds.
- [x]{.pnum} *`N`* be `min(@_M_@, result_last - result)`.
:::

[16]{.pnum} *Preconditions*: The ranges `[first, @[last]{.rm}[first + _N_]{.add}@)` and
...
`iterator_traits<ForwardIterator1>​::​value_type` is not *Cpp17MoveConstructible* (Table 31). — *end note*]

::: add
[*Note 2*: For the overload with an *`execution-policy`*, there might be a performance cost if
`iter_value_t<I>` is not `move_constructible`. — *end note*]
:::

[17]{.pnum} *Effects*: Copies all of the elements referred to by the iterator `i` in the range
`[first, @[last]{.rm}[first + _N_]{.add}@)` for which *`E`* is `true`.

[18]{.pnum} *Returns*:

- [18.1]{.pnum} `result + @_N_@` for the overloads in namespace `std`.
- [18.2]{.pnum} `{@[last]{.rm}[first + _N_]{.add}@, result + @_N_@}` for the overloads in namespace `ranges`.

[19]{.pnum} *Complexity*: Exactly `last - first` applications of the corresponding predicate and any projection.
...
performs `*(result + @_n_@) = std​::​​move(*(first + @_n_@))`.

[9]{.pnum} *Returns*: `result + @_N_@`.
