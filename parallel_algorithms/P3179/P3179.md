...
## Corner cases: `reverse_copy` and `rotate_copy` {#corner_cases}
...
auto res = std::ranges::copy(input, output);
```
...
auto res = std::ranges::rotate_copy(input, input.begin() + 3, output);
```
...
// TODO: Perhaps also provide design rational for partition_copy.
...
- [x.5]{.pnum} *`i1`* be the rightmost iterator in `[first, last)` for which *`E`* holds and `out_true == last_true`, if
  such an iterator exists. Otherwise, let *`i1`* be `last`;
- [x.6]{.pnum} *`i2`* be the rightmost iterator in `[first, last)` for which *`E`* is `false` and `out_false == last_false`,
  if such an iterator exists. Otherwise, let *`i2`* be `last`;
- [x.7]{.pnum} *`N`* be `min(@_j1_@ - first, @_j2_@ - first)`.
...
[17]{.pnum} *Effects*: For each iterator `i` in `[first, @_N_@)`, copies `*i` to the output range beginning with `out_true`
if `@_E_@(*i)` is `true`, or to the output range beginning with `out_false` otherwise.

[18]{.pnum} *Returns*:

- [18.1]{.pnum} `{last_true, last_false}` for the overloads in namespace `std`.
- [18.2]{.pnum} `{first + @_N_@, o1, o2}` for the overloads in namespace `ranges`, where o1... // TODO

[19]{.pnum} *Complexity*: At most `first + @_N_@` applications of `pred` and `proj`.
:::
