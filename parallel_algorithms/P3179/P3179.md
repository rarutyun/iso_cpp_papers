...
- All input and output data sequences must be bounded. See [](#require_bounded_ranges).
  - As a consequence, when the bounded output has less space than needed for all the input, an algorithm might return
      iterators pointing to the positions past the last processed elements.
...
- `rotate_copy`, `reverse_copy`, and `partition_copy` design is described separately. See [](#corner_cases).
...
(One possible deviation from this principle, `partition_copy`, is discussed separately [below](#partition_copy).)
...
### Algorithms with semantic adjustments ### {#corner_cases}
...
Because of that, for several algorithms described below we propose modifications to which iterators are returned
that do not match their existing serial counterparts. If some day serial range algorithms with range-as-the-output
appear, we envision the same semantics for these algorithms.
...
As a general principle, if a view satisfies the _`random-access-sized-range`_ concept, it can be used with parallel
range algorithms; the same applies to a view pipeline. There are additional questions to explore, and it is possible
that some views - standard or custom - cannot be safely used in a parallel context. However we think that these questions
have no direct impact on the proposed design, and should better be considered in a follow-up paper.
We present our initial analysis and exploration scope in the [](#parallelism_and_views) subsection below.
...
## Parallelism implications for views ## {#parallelism_and_views}

We need to provide an analysis of using views in parallel context. Even though, we require `random_access_range` for
Parallel Range Algorithms API, data races are still possible.
...
Today range-based overloads are specified to call the "iterator and sentinel" overloads, but it might be useful to extend
allowed operations directly on views in parallel context. Such an extension gives the implementation freedom, which
might be useful for accelerators (e.g., GPUs).

In particular, we think that the operations listed below are the the minimal list for analysis:

- `begin()/end()`
- `size()` for sized ranges
- copy constructor
- `operator[]`, if available

Besides that we need to cover the implications of sharing a state between `view` and `iterator` on the possibility
of data races. One example is `transform_view`, where iterators keep pointers to the function object that is stored
in the view itself.

Going forward, we want to analyze views that cache their state. The example is `reverse_view`, which caches the end even
when *`base_`* is `random_access_range`.

Also, if we extend the allowed operations on views in parallel context we should add the analysis, which
view pipelines are considered to be thread-safe to do such operations. For example:
...
If we then use `ref` in parallel algorithms and allow copy operations for views in parallel context the pipeline above
potentially leads to data races because every copy will operate on `reverse_view`, which would cache the state concurrently
on the `begin()` call but `reverse_view` itself does not guarantee thread safe `begin()` access.

To conclude:

- This proposal does not require any design changes based on our analysis of views thread safety because
  parallel range algorithms can be implemented by calling `ranges::begin` and `ranges::end()` in serial context and
  then work with iterators.
- However, other implementation strategies might also be useful, specifically for heterogeneous implementation (e.g.,
  for GPUs), thus we would like to bring the analysis what operations directly on views are useful in parallel context.
- Our plan is to cover allowed operations on views in parallel context in a separate paper.
...
[3]{.pnum} *Effects*: Copies [all the elements of the two ranges]{.rm}[first `@_K_@` elements of the range]{.add}
`[first1, last1)` and [first `@_N_ - _K_@` elements of the range]{.add} `[first2, last2)` into the range
...
the final `max(@_n_@ − @_m_@, 0)` elements from the second range are copied to the output range, in order.
