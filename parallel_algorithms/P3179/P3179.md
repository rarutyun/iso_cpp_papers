...
- [x.1]{.pnum} `result_last` be `result + (last - first)` for the overloads with no parameter `result_last` or `result_r`;
- [x.2]{.pnum} *`M`* be the number of iterators `i` in `[first, last)` for which `@_E_@(i)` is `false`;
...
- [13.2]{.pnum} `{@[last]{.rm}[j]{.add}@, result + @_N_@}`, for the algorithms in namespace `ranges`[,
  where `j` is the iterator in `[first, last)` for which `@_E_@(j)` is `false` and there are exactly *`N`*
  iterators `i` in `[first, j)` for which `@_E_@(i)` is `false`, or `j == last` if no such iterator exists]{.add}.
