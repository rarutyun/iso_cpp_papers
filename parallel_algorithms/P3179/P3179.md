...
- [x.3]{.pnum} *`M`* be the number of iterators `i` in the range `[first, last)` for which the condition `@_E_@(i)` holds;
...
- [18.2]{.pnum} `{@[last]{.rm}[j]{.add}@, result + @_N_@}` for the overloads in namespace `ranges`[,
  ...
  iterators `i` in `[first, j)` for which `@_E_@(i)` holds, or `j == last` if no such iterator exists]{.add}.
