...
- [x.1]{.pnum} *`M`* be the number of iterators `i` in `[first, last)` for which `@_E_@(*i)` holds, and *`K`* be `last - first - M`;
...
- [x.1]{.pnum} *`i1`* be the iterator in `[first, last)` for which `@_E_@(*@_i1_@)` holds and there are exactly *`M`*
  iterators `j` in `[first, @_i1_@)` for which `@_E_@(*j)` holds, or `last` if no such iterator exists;
...
[17]{.pnum} *Effects*: For each iterator `i` in ``[first, @@[last]{.rm}[`first + @_N_@`]{.add}@@)``, copies `*i` to theoutput range
...
- TODO: Possibly improve the wording for `merge`. DO NOT FORGET TO REMOVE THIS BULLET !!!!
