...
Below we list of algorithms where we faced design issue(s). We would like to add them later:
...
::: add
[x]{.pnum} Let *`NEW_FIRST`* be `first + (last - first) - @_N_@`.
:::

[6]{.pnum} *Preconditions*: The ranges `[@[first]{.rm}[*NEW_FIRST*]{.add}@, last)` and `[result, result + @_N_@)`
do not overlap.

[7]{.pnum} *Effects*: Copies the range `[@[first]{.rm}[*NEW_FIRST*]{.add}@, last)` to the range
`[result, result + @_N_@)` such that for every non-negative integer `i < @_N_@` the following assignment takes place:
`*(result + @_N_@ - 1 - i) = *(@[first]{.rm}[*NEW_FIRST*]{.add}@ + i)`.

[8]{.pnum} *Returns*:

- [8.1]{.pnum} `result + @_N_@` for the overloads in namespace `std`.
- [8.2]{.pnum} ``{@@[last]{.rm}[`@_NEW_FIRST_@`]{.add}@@, result + @_N_@}`` for the overloads in namespace `ranges`.
...
- [x.6]{.pnum} *`K`* be the rightmost position of an element `e2` in `[first2, last2)` such that
  the number of elements `e1` in `[first1, last1)` for which *`E`* holds is less than or equal to *`N - K`*.
:::
...
- Add necessary wording to [algorithm.syn]{- .sref}
- Add the contextual algorithms description to the wording, even if no changes are made.
- Add *`random-access-sized-range`* an exposition-only concept for wording simplification
- Suggest to bump old feature testing macro as was voted in LEWG
- Shorten the `ExecutionPolicy` parameter, italicize *`execution-policy`* concept in wording.
- Support `result_last` or similar for the algorithms where range-as-an-output and a novelty
...
- Modify `fill` and `generate` algorithms family to use `indirectly_writable` instead of `output_iterator` and
  `output_range` for design consitency.
- Add sentinel-for-output for `copy_n` algorithm family.
- Add short descriptions of the new exposition-only concepts to the design overview.
