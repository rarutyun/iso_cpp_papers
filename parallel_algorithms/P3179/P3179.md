...
For each iterator `i` and sentinel `s` produced from range `r`, the semantics of `s - i` is either `ranges::distance(r)` or
`ranges::distance(i, s)`.
:::
...
algorithm overload has an additional]{.rm} function parameter of type [`ExecutionPolicy&&`]{.rm} [`T&&`, where `T` is the
execution policy template parameter]{.add}, [which is the]{.rm}[as an additional]{.add} first function parameter [P]{.add}.
...
[that template parameter satisfies *`execution-policy`* concept]{.add}.
