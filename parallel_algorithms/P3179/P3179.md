...
- Two corner cases: `rotate_copy` and `reverse_copy` design is described separately. See [](#corner_cases).
...
### Corner cases for bounded ranges ### {#corner_cases}

#### `reverse_copy` design #### {#reverse_copy}

`reverse_copy` algorithm is special because it traverses an input range from the end to the beginning for serial algorithms.
...
It returns `last` iterator for input because it assumes the memory under `result` is always sufficient to write everything.

In the 'Parallel Ranges Algorithms' overloads the situation is different. Since the output is bounded the input might be
...
The only reasonable semantics we could think of is returning an iterator to the last inserted element in the reverse order
starting from the `last - 1` element, when the output size is not sufficient. The implication of this decision is that the
algorithm would return `first` in case output size is sufficient to write everything from input.

Example with ranges-based signature:
...
auto res = std::ranges::copy(@_policy_@, input, output);
```

This design choice is made the users need the information about how many elements were handled. Returning `last` does not
make sense for such a signature.
...
// After the algorithm execution:
// - output is {4,3}
// - res.in points to the element equal to 2, however it has reverse_iterator type,
//   thus not comparable with input.end() type
// - res.in.base() has input.end() type, res.in.base() == input.end() - 2 is true
auto res = std::ranges::copy(input | std::views::reverse | std::views::take(2), output.begin());
```

But in the example above we bound input. Let's use `uninitialized_copy` where we can bound output:

```cpp
std::unique_ptr<int[]> mem(static_cast<int*>(operator new[](10)));
...
//   thus not comparable with input.end() type
// - res.in.base() has input.end() type, res.in.base() == input.end() - 2 is true
auto res2 = std::ranges::uninitialized_copy(input | std::views::reverse, mem_range | std::views::take(2));
```

If some day serial range algorithms with range-as-an-output appear we envision the same semantics for them.

#### `rotate_copy` design #### {#rotate_copy}

The situation is very similar to `reverse_copy` in a sense that `rotate_copy` does not traverse an input in order.
...
`last` for an input range because it also assumes that the output size is always sufficient.

For 'Parallel Ranges Algorithms' overloads the situation is again different. The output size might be insufficient to
write everything. We believe that we also need to return the iterator which is past-to-last we were able to insert. The
implication of this decision is the algorithm would return `middle` for input when output size is sufficient to write
everything from the input.

Let's consider the example with range-based signature:
...
// After the algorithm execution:
// - output is {4,5}
// - res.in points to the element equal to 6, exactly past-to-last written to the output
auto res = std::ranges::rotate_copy(@_policy_@, input, input.begin() + 3, output);
```

Another example when the output size is sufficient for the subrange from `middle` to `last` but not sufficient to write all
the input:
...
// After the algorithm execution:
// - output is {4,5,6,1,2}
// - res.in points to the element equal to 3, exactly past-to-last written to the output
auto res = std::ranges::rotate_copy(@_policy_@, input, input.begin() + 3, output);
```
...
For the first corner case we propose to return `first` because this is the iterator that would have been written next (in
other words, part-to-last), if the space would be sufficient.
...
// After the algorithm execution:
// - output is {4,5,6}
// - res.in points to the element equal to 1, past-to-last written to the output from this algorithm standpoint
auto res = std::ranges::rotate_copy(@_policy_@, input, input.begin() + 3, output);
```

For the second corner case we propose to return `middle`. This situation is distinguishable from the case when the output
size is sufficient because `result` was not moved anywhere. So the returned object is `{middle, result}`, while in case when
...
// - res.out == result is true
auto res = std::ranges::rotate_copy(@_policy_@, input, input.begin() + 3, output);
```

Please note that this `rotate_copy` parallel range algorithm never returns `last`.
...
`true`, another one - for predicate returning `false`. The existing overloads assume that the output is
always sufficient, thus it's guaranteed to traverse the whole input. However, this is not the case for the proposed
...
The biggest question is when to stop to traverse the input if one of the output ranges does not have memory to write into.

There are three considered options:

1. Stop when both ranges do not have space. It means that if, for example, `out_true` range reaches the end but the
   `out_false` still has some available space the algorithm continues going through the input. Given that the algorithm
   returns the iterator of input where it stops this is bad semantics in our view because it creates a false impression that
   the input till returned iterator is handled, while, in fact, some elements might be skipped. This semantics violates user
   expectations.
1. Stop when one of the output ranges reaches the end. While this semantics is perhaps good enough, we still could do better
   because users might have, for example, the `out_true` that reaches the end, but the predicate returns `false` for the rest
   of the input, so we can still write `out_false` range since it did not reach its end.
1. Stop when we cannot write anymore. It means that if we still have input to traverse we always take the element from it,
   check what is returned by the predicate and stop only if we don't have available space in the respective output range:
   `out_true` for `true` result, `out_false` for `false` result.

We propose option (3).

Possible serial implementation of the proposed semantics:
...
Example for range-based signature:
...
// - res.in points to the element equal to 7 because this is the first element we cannot insert
auto res = partition_copy(@_policy_@, in, out_true, out_false, pred);
```

With the semantics (1) the example above would stop at the `in.end()` pretending that it handles everything.
However, this is not true because it skips elements equal to `7` and `9`. Thus, we do NOT propose this semantics.

With the semantics (2) the example above would stop at the element equal to `2` because the `out_true` range reaches its end.
...
- Add necessary wording modifications to `partition_copy` and `set_` algorithms family (like `set_union`, etc.)
...
- Should it be allowed for parallel algorithms to copy views?
- Should `begin()/end()/size()` and possibly other operations with ranges be considered
  like element access functions for the purpose of thread safety definition?
...
[algorithms.parallel.exec]{- .sref} to make arbitrary copies of elements from the input sequence.
...
[x]{.pnum} Let `result_last` be `result + (last - first)` for the overloads with no parameter `result_last` or `result_r`.
...
[x]{.pnum} Let `result_last` be `result + (last - first)` for the overloads with no parameter `result_last`.
:::
...
- [18.2]{.pnum} `{@[last]{.rm}[j]{.add}@, result + @_N_@}` for the overloads in namespace `ranges`[,
  where the iterator `j` points to a position in the range `[first, last)` past the last copied element
  if `result + @_N_@ == result_last`, and `j == last` otherwise]{.add}.
...
## Modify `swap_ranges` in [alg.swap]{- .sref} ## {#modify_swap}
...
::: add
- [1.x]{.pnum} `result_last` be `result + M` for the overloads with no parameter `result_last` or `result_r`;
- [1.x]{.pnum} *`N`* be `min(@_M_@, result_last - result)`;
:::
- [1.3]{.pnum} *`E`* be
...
to the overload with an `ExecutionPolicy`.]{.rm}
...
[8]{.pnum} *Preconditions*: The ranges `[first, @[last]{.rm}[first + _N_]{.add}@)` and
``[result, result + @[`(last - first)`]{.rm}[_N_]{.add}@)`` do not overlap.
...
- [x.1]{.pnum} `result_last` be `result + (last - first)` for the overloads with no parameter `result_last` or `result_r`;
- [x.2]{.pnum} *`M`* be the number of elements in `[first, last)` for which *`E`* is `false`;
- [x.3]{.pnum} *`N`* be `min(@_M_@, result_last - result)`.
:::
...
[12]{.pnum} *Effects*: Copies [all the]{.rm}[`@_N_@`]{.add} elements referred to by the iterator `i` in the range
`[first, last)` for which *`E`* is `false` [into the range `[result, result + @_N_@)`]{.add}.

[13]{.pnum} *Returns*:

- [13.1]{.pnum} `result + @_N_@`, for the algorithms in namespace `std`.
- [13.2]{.pnum} `{@[last]{.rm}[j]{.add}@, result + @_N_@}`, for the algorithms in namespace `ranges`[, where
  the iterator `j` points to a position in the range `[first, last)` past the last copied element
  if `result + @_N_@ == result_last`, and `j == last` otherwise]{.add}.
...
- [x.1]{.pnum} `result_last` be `result + (last - first)` for the overloads with no parameter `result_last` or `result_r`;
- [x.2]{.pnum} *`M`* be the number of elements in the range `[first + 1, last)` for which *`E`* is `false`.
...
[[*Note 1*: For the overloads with an `ExecutionPolicy`, there might be a performance cost if the value type of
`ForwardIterator1` does not meet both the *Cpp17CopyConstructible* and *Cpp17CopyAssignable* requirements.
For the overloads with an *`execution-policy`*, there might be a performance cost if
`iter_value_t<I>` does not meet both the `copy_constructible` and `copy_assignable`. — *end note*]]{.add}

[9]{.pnum} *Effects*: Copies only the first element from [every]{.rm}[*`N`*]{.add} consecutive group[s]{.add} of equal
elements referred to by the iterator `i` in the range `[first, last)` for which *`E`* holds [into a range
`[result, result + @_N_@)`]{.add}.

[10]{.pnum} *Returns*:

- [10.1]{.pnum} `result + @_N_@` for the overloads in namespace `std`.
- [10.2]{.pnum} `{@[last]{.rm}[j]{.add}@, result + @_N_@}` for the overloads in namespace `ranges`[, where the iterator `j`
points to a position in the range `[first, last)` past the last copied element if `result + @_N_@ == result_last`, and
`j == last` otherwise]{.add}.
...
[x]{.pnum} *Preconditions*: The ranges `[@*NEW_FIRST*@, last)` and `[result, result + @_N_@)` do not overlap.
...
[x]{.pnum} Let *`M`* be `last - first` and *`N`* be `min(@_M_@, result_last - result)`.
...
an [`ExecutionPolicy` template parameter]{.rm}[execution policy]{.add}, does so in order starting from `i = 0` and proceeding to
`i = (last - first) - n - 1`.
...
- [6.2]{.pnum} for the overloads in namespace `ranges` [without an *`execution-policy`*]{.add}, `I` models
...
[5]{.pnum} *Complexity*: For the overloads with no [`ExecutionPolicy`]{.rm}[execution policy]{.add}, linear on average.
For the overloads with an [`ExecutionPolicy`]{.rm}[execution policy]{.add}, `@_O_@(@_N_@)` applications of the predicate,
and `@_O_@(@_N_@log@_N_@)` swaps, where `@_N_@ = last - first`.
...
- [8.1]{.pnum} For the overload with no [`ExecutionPolicy`]{.rm}[execution policy]{.add}, exactly *`N`* applications of the
  predicate and projection. At most `@_N_@ / 2` swaps if the type of `first` meets the *Cpp17BidirectionalIterator*
  requirements for the overloads in namespace `std` or models `bidirectional_iterator` for the overloads in namespace
  `ranges`, and at most *`N`* swaps otherwise.
- [8.2]{.pnum} For the overload with an [`ExecutionPolicy`]{.rm}[execution policy]{.add}, `@_O_@(@_N_@log@_N_@)` swaps
  and `@_O_@(@_N_@)` applications of the predicate.
...
- [13.1]{.pnum} For the overloads with no [`ExecutionPolicy`]{.rm}[execution policy]{.add}, at most `@_N_@log@~2~_N_@`
  swaps, but only `@_O_@(@_N_@)` swaps if there is enough extra memory. Exactly *`N`* applications of the predicate and projection.
- [13.2]{.pnum} For the overload[s]{.add} with an [`ExecutionPolicy`]{.rm}[execution policy]{.add}, `@_O_@(@_N_@log@_N_@)` swaps
  and `@_O_@(@_N_@)` applications of the predicate.
...
- [x.1]{.pnum} `proj` be `identity{}` for the overloads with no parameter named `proj` and let `@_E_@(@_x_@)` be
  `bool(invoke(pred, invoke(proj, x)))`;
- [x.2]{.pnum} *`M`* be the number of elements for which `@_E_@(@_x_@)` holds, `K` be `last - first - @_M_@`;
- [x.3]{.pnum} `last_true` be `out_true + @_M_@` for the overloads with no parameter `last_true` or `out_true_r`;
- [x.4]{.pnum} `last_false` be `out_false + @_K_@` for the overloads with no parameter `last_false` or `out_false_r`;
- [x.5]{.pnum} *`i1`* be the rightmost iterator in `[first, last)` for which *`E`* holds and there is an iterator
  `out_true_it` such that `out_true_it == last_true` is `true`. If such iterators do not exist, let *`i1`* be `last`;
- [x.6]{.pnum} *`i2`* be the rightmost iterator in `[first, last)` for which *`E`* is `false` and there is an iterator
  `out_false_it` such that `out_false_it == last_false` is `true`. If such iterators do not exist, let *`i2`* be `last`;
- [x.7]{.pnum} *`N`* be `min(@_i1_@ - first, @_i2_@ - first)`.
:::
...
meet the *Cpp17CopyConstructible* requirements. [For the overloads with an *`execution-policy`*, there might be a performance
cost if `first`'s value type does not meet the `copy_constructible` requirements.]{.add} — *end note*]

[17]{.pnum} *Effects*: For each iterator `i` in ``[first, @@[last]{.rm}[`first + @_N_@`]{.add}@@)``, copies `*i` to the output range
beginning with `out_true` if `@_E_@(*i)` is `true`, or to the output range beginning with `out_false` otherwise.
...
`out_false`]{.rm}[be the iterator past the last copied element in the output range `[out_false, last_false)`]{.add}. Returns
...
[19]{.pnum} *Complexity*: [Exactly `last - first`]{.rm}[At most `first + @_N_@`]{.add} applications of `pred` and `proj`.
...
- [x.1]{.pnum} *`M`* be `(last1 - first1) + (last2 - first2)`;
...
- [x.4]{.pnum} *`N`* be `min(@_M_@, result_last - result)`;
- [x.5]{.pnum} *`E`* be `bool(invoke(comp, invoke(proj2, e2), invoke(proj1, e1)))`;
- [x.6]{.pnum} *`K`* be the smallest integer in `[0, last1 - first1)` such that for the element `e1` in the position
...
[3]{.pnum} *Effects*: Copies [all the elements of the two ranges]{.rm}[`@_K_@` elements of the range]{.add}
`[first1, last1)` and [`@_N_ - _K_@` elements of the range]{.add} `[first2, last2)` into the range
...
- [5.1]{.pnum} For the overloads with no [`ExecutionPolicy`]{.rm}[execution policy]{.add}, at most `@_N_@ − 1` comparisons
  and applications of each projection.
- [5.2]{.pnum} For the overloads with an [`ExecutionPolicy`]{.rm}[execution policy]{.add}, `@_O_@(@_N_@)` comparisons
...
- [11.1]{.pnum} For the overloads with no [`ExecutionPolicy`]{.rm}[execution policy]{.add}, and if enough additional memory
  is available, exactly `@_N_@ − 1` comparisons.
...
- [x.2]{.pnum} *`K`* be the number of elements in `[first2, last2)` that are not present in `[first1, last1)`;
- [x.3]{.pnum} `result_last` be `result + (last1 - first1) + @_K_@` for the overloads with no parameter `result_last` or `result_r`;
- [x.4]{.pnum} *`N`* be `min((last1 - first1) + @_K_@, result_last - result)`.

::::: note
If `[first1, last1)` contains *`m`* elements that are equivalent to each other and `[first2, last2)` contains
*`n`* elements that are equivalent to them, then the last `max(@_n_@ − @_m_@, 0)` equivalent elements
from the second range are not present in the first range.
:::::
:::
...
- [4.1]{.pnum} `result_last` for the overloads in namespace `std`.
- [4.2]{.pnum} [`{last1, last2, result_last}`]{.rm}[`{j1, j2, result + @_N_@}`]{.add} for the overloads in namespace `ranges`[,
  where the iterators `j1` and `j2` point to positions past the last copied or skipped elements in `[first1, last1)` and `[first2, last2)`
  respectively if `result + @_N_@ == result_last`, and `j1 == last1`, `j2 == last2` otherwise]{.add}.

[5]{.pnum} *Complexity*: At most `2 * ((last1 - first1) + (last2 - first2)) - 1` comparisons and applications of each
projection.

[6]{.pnum} *Remarks*: Stable ([algorithm.stable]{- .sref}). If `[first1, last1)` [and `[first2, last2)`]{.add} contain[s *`m`*]{.rm}
elements that are equivalent to each other [and `[first2, last2)` contains *`n`* elements that are equivalent to them]{.rm},
then [all *`m`*]{.rm}[equivalent]{.add} elements from the first range are copied to the output range, in order, and then
the [final `max(@_n_@ − @_m_@, 0)`]{.rm} elements from the second range [not present in the first range]{.add}
...
- [x.1]{.pnum} `comp` be `less{}`, and `proj1` and `proj2` be `identity{}` for the overloads with no parameters by those
  names;
- [x.2]{.pnum} *`K`* be the number of elements in `[first1, last1)` that are present in `[first2, last2)`;
- [x.3]{.pnum} `result_last` be `result + @_K_@` for the overloads with no parameter `result_last` or `result_r`;
- [x.4]{.pnum} *`N`* be `min(@_K_@, result_last - result)`.
:::
...
- [4.1]{.pnum} `result_last` for the overloads in namespace `std`.
- [4.2]{.pnum} [`{last1, result_last}`]{.rm}[`{j1, result + @_N_@}`]{.add} for the overloads in namespace `ranges`[,
  where the iterator `j1` points to the position past the last copied element in `[first1, last1)` if
  `result + @_N_@ == result_last`, and `j1 == last1` otherwise]{.add}.
...
- [4.2]{.pnum} [`{last1, last2, result_last}`]{.rm}[`{j1, j2, result + @_N_@}`]{.add} for the overloads in namespace `ranges`[,
  ...
  and `[first2, last2)` respectively if `result + @_N_@ == result_last`, and `j1 == last1`, `j2 == last2` otherwise]{.add}.
...
- TODO: Explain design choice for `rotate_copy` and `reverse_copy`
- TODO: Add wording for `set_` algorithms family (that makes wording complete)
- TODO: Add wording for `partition_copy` algorithms family (that makes wording complete)
...
- Support `result_last` or similar for the algorithms where range-as-the-output is a novelty
  (`copy`, `transform`, etc.), unless stated otherwise in [](#issues_to_address) or in [](#out_of_scope)
...
- Thanks to Mikhail Dvorsky for the implementation experience.
