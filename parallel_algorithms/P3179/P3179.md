...
[How does LWG want to refer to `@_execution-policy_@ Ep`? Like suggested? Without `Ep`? Without *`execution-policy`*?]{.draftnote audience="LWG"}
...
[If we add "in the `std` namespace" as requested in a previous LWG review, would the requirement still apply
to the linear algebra algorithms in the `std::linalg` namespace?]{.draftnote audience="LWG"}
...
```
[In the synopsis, the declaration of `find_last` does not set the default type for `T`.
In [alg.find.last], it does: `class T = projected_value_t<I, Proj>`.]{.draftnote audience="LWG"}
```diff
...
+  template<@_execution-policy_@ Ep, @_sized-random-access-range_@ R, random_access_iterator OutR,
...
[*Note X*: The overloads in namespace `ranges` require `Fun` to model `copy_constructible`. — *end note*]
[LWG previously requested to remove the note about `Fun` modelling `copy_constructible`. However, it is a copy
of the existing note for `ranges::for_each` that can be seen above. For consistency, should we keep or remove
all these notes (also for `ranges::for_each_n`)?]{.draftnote audience="LWG"}
...
[A dot is missed at the end of the sentence for consistency with the rest of [alg.starts.with].]{.draftnote audience="LWG"}
:::
...
[A dot is missed at the end of the sentence for consistency with the rest of [alg.ends.with].]{.draftnote audience="LWG"}
:::
...
```
[Should we change `policy` to `exec` above, for consistency with every other place?]{.draftnote audience="LWG"}
...
[17]{.pnum} *Effects*: Copies [all of the]{.rm}[first *`N`*]{.add} elements referred to by the iterator `i` in the range
...
[12]{.pnum} *Effects*: Copies [all the]{.rm}[first *`N`*]{.add} elements referred to by the iterator `i` in the range
...
for which `@_E_[(i)]{.add}@` holds [into a range `[result, result + @_N_@)`]{.add}.
[Modifications in the middle of the sentence are to fix the wording by aligning with `unique`.]{.draftnote audience="LWG"}

[10]{.pnum} *Returns*:

- [10.1]{.pnum} `result + @_N_@` for the overloads in namespace `std`. [*`N`* was not defined previously, now it is.]{.draftnote audience="LWG"}
- [10.2]{.pnum} `{@[last]{.rm}[j]{.add}@, result + @_N_@}` for the overloads in namespace `ranges`[, where `j` is
the iterator in `[first + 1, last)` for which `@_E_@(j)` is `false` and there are exactly `@_N_@ - 1` iterators `i`
in `[first + 1, j)` for which `@_E_@(i)` is `false`, or `j == last` if no such iterator exists]{.add}.
...
[It seems that "Returns:" is missed here.]{.draftnote audience="LWG}
...
[Should it be "`@_O_@(@_N_@log@_N_@)` comparisons, and twice as many projections as comparisons"?]{.draftnote audience="LWG"}
...
[It seems that ":" is missed here.]{.draftnote audience="LWG}
...
- [4.2]{.pnum} [`{last1, result_last}`]{.rm}[`{j1, result + @_N_@}`]{.add} for the overloads in namespace `ranges`[,
  where the iterator `j1` points to the position past the last copied or skipped element in `[first1, last1)` if
...
- Update the design description and the wording to reflect the LEWG decision on where an algorithm with bounded output
  should stop
- Mark new overloads as *freestanding-deleted*
- Per LWG guidance, rename the key concept to *`sized-random-access-range`*
- Improve the wording for `copy_if`, `remove_copy[_if]`, `unique_copy`, and `merge`
- Add a few more LWG questions
