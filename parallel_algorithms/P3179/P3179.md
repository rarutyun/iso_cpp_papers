...
have no direct impact on the proposed design, and should better be considered in a follow-up paper. The reason is that
these potential problems are not novel for C++ standard. They same questions apply if we pass iterators taken from
views to the existing C++17 parallel algorithms.
...
of such use. Some investigation was done by other authors in [@P3159R0], however it is primarily
...
Views are also sometimes used via iterators with C++17 parallel algorithms; see examples in [](#motivation).
...
The current description in [algorithms.parallel.defns]{- .sref} may need to be extended to better explain the impact on
views. However, this question also applies to the existing C++17 parallel algorithms, thus the design of this paper does not
...
`std::ranges::begin`, `end`, and `size` which are guaranteed by the _`random-access-sized-range`_ concept.
...
We plan to consider all these questions in detail in a separate paper.
