...
execution policy template parameter]{.add}, [which is the]{.rm}[as the]{.add} first function parameter [P]{.add}.
...
[3]{.pnum} Unless otherwise specified, the complexity requirements of [`ExecutionPolicy`]{.rm}[parallel]{.add} algorithm
overloads are relaxed from the complexity requirements of the [corresponding]{.add} overloads without [the parameter P]{.add}
as follows: when the guarantee says “at most *expr*” or “exactly *expr*” and does not specify the number of assignments or
swaps, and *expr* is not already expressed with *O*() notation, the complexity of the algorithm shall be *O*(*expr*).
...
there [might]{.rm}[can]{.add} be a performance cost if `iterator_traits<ForwardIterator1>::value_type` is not
*Cpp17MoveConstructible* (Table 31). [For the parallel algorithm overloads in namespace `ranges`, there can be a performance
cost if `iter_value_t<I>` is not `move_constructible`.]{.add} — *end note*]
...
be a performance cost if `iter_value_t<I>` is not `move_constructible`.]{.add} — *end note*]
...
parallel algorithm overloads in namespace `ranges`, there can be a performance cost if `iter_value_t<I>` does not meet both
the `copy_constructible` and `copy_assignable`. — *end note*]]{.add}
...
<!-- internal TODOs:
think about Tomasz's suggestion about copy_if wording.
SUGGESTION: TK’s chat: “18.2 {last, result + N} if N == M
18.2 {j, result + N} if j is iterator // without the or” -->
